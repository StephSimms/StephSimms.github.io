...
function showSlides(n) {
    ...
    var dots = document.getElementsByClassName("dot");//This takes all elements with the class name "dot" and stores them in the variable arrays "dots"
    ...
    dots[slideIndex -1].className += " active";//This adds the active styling to the dots associated with the image
}
...
document.addEventListener("click", function(event) {
    //Here we state that if the click happens on the cancel button OR anywhere that is not the contaxct form AND the click does not happen on any element with the contact class then call the closeForm() function
